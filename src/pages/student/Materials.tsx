    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="lede">{t('student.materialsSub', { group: group.name })}</p>
        </div>
      </div>
        ...
        return (
          <div key={month.id} style={{ marginBottom: 20 }}>
            <h2 style={{ fontSize: '1.1rem', margin: '18px 0 10px' }}>
              {t('common.month')} {month.index} · {month.name}
            </h2>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {weekBooks.map((b) => (
                        <div key={b.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10 }}>
                          <div>
                            ...
                            <div style={{ fontSize: '0.78rem', color: 'var(--muted)' }}>{b.author} · {b.chapters}</div>
                          </div>
                      ...
                      {weekMaterials.map((m) => (
                        <div key={m.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10 }}>
                          ...
                          <button className="btn" onClick={() => downloadPlaceholder(m.title, 'Additional weekly material.')}>
                            {t('common.open')}
                          </button>
                        </div>
                      ))}
