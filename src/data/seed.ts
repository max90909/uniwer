...
/**
 * Версия демо-набора. Поднимайте её при любом изменении структуры или состава
 * сидов — store сверяет её с сохранённым снимком и, если версии разошлись,
 * пересобирает данные заново.
 *
 * Без этого снимок в localStorage жил вечно: у тех, кто открывал стенд раньше,
 * оставались старые группы («Группа A») и тексты без ключей перевода, поэтому
 * месяцы и материалы не переводились при смене языка — правки сидов до них
 * просто не доходили.
 */
